    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('user.update',['user'=>$user->id]) }}" class="form-horizontal" method="post">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('name') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('name',$user->name) }}" name="name" class="form-control" id="name" placeholder="Enter Name">
                                @error('name')
                        ...
                        <div class="form-group row {{ $errors->has('username') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('username',$user->username) }}" name="username" class="form-control" id="username" placeholder="Enter Username">
                                @error('username')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('mobile_no') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('mobile_no',$user->mobile_no) }}" name="mobile_no" class="form-control" id="mobile_no" placeholder="Enter Mobile No.">
                                @error('mobile_no')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
